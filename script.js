...
function addGender() {
  if (document.querySelector('#custom-gender').checked) {
    ...
    form.insertBefore(newInput, parag);
...
function showFields() {
  ...
  const greetings = `Olá, ${dataList[0].value}, seja bem vindo(a)!`;
...
function validateForm(event) {
  ...
  const change = document.querySelector(".right-content");
  change.classList.toggle("welcome");
